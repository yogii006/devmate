...
// Allow runtime configuration via window._env_.REACT_APP_API_URL (generated by container entrypoint)
// Behavior:
// - If runtime `window._env_.REACT_APP_API_URL` is defined (even as an empty string), use it.
// - Else if build-time `process.env.REACT_APP_API_URL` is defined, use it.
// - Else: when running on localhost (dev) default to http://localhost:8000, otherwise use empty string
const getApiRoot = () => {
  // runtime config (injected by docker entrypoint)
  if (typeof window !== 'undefined' && window._env_ && Object.prototype.hasOwnProperty.call(window._env_, 'REACT_APP_API_URL')) {
    return window._env_.REACT_APP_API_URL; // may be empty string => same-origin
  }

  // build-time env (CRA)
  if (typeof process !== 'undefined' && typeof process.env !== 'undefined' && typeof process.env.REACT_APP_API_URL !== 'undefined') {
    return process.env.REACT_APP_API_URL;
  }

  // default: during local development, talk to localhost:8000; in deployed monolith use same-origin (empty)
  try {
